...
export default function Player() {
  ...
  useFrame((state, delta) => {
    ...
    const impulseStraight = 0.6 * delta;
    const torqueStraight = 0.2 * delta;

    if (forward) {
      impulse.z -= impulseStraight;
      torque.x -= impulseStraight;
    }

    if (backward) {
      impulse.z += impulseStraight;
      torque.x += torqueStraight;
    }

    if (leftward) {
      impulse.x -= impulseStraight;
      torque.z += torqueStraight;
    }
